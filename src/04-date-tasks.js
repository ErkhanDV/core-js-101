...
function timeSpanToString(startDate, endDate) {
  ...
  return new Date(diff).toISOString().slice(-13, -1);
}
...
function angleBetweenClockHands(date) {
  const hours = new Date(date).getUTCHours() > 11
  ...
  const result = Math.abs(hoursAngle - minutesAngle) > 180
    ? Math.abs(hoursAngle - minutesAngle) - 180
